...
const patchedMarkdownMediaLocalState =
  "x=l??``,S=codexLinuxResolveMarkdownMediaPath(x,codexLinuxMarkdownCwd),C=x.length>0,w=Zt(x),T,ke=codexLinuxNormalizeMarkdownRemoteMediaUrl(x);t[0]===x?T=t[1]:(T=Yt(S??ke??x),t[0]=x,t[1]=T);let E=T,O=E===`video`,k=!1,A=S!=null,j=an(x,w==null&&C&&E===`image`&&!A&&tn(x)),M=!1,N=O?(A?me(S):ke):null,P=!O&&A,{data:I,isLoading:L}=F(`read-file-binary`,{params:{path:P?S??x:``,hostId:o},queryConfig:{enabled:P,gcTime:1/0,staleTime:1/0}});";
...
const patchedMarkdownMediaResolvedSourceState =
  "let te=I?.contentsBase64??null,R=j.safeUrl??N??(P&&te!=null?$t({contentsBase64:te,mimeType:I?.mimeType??null,path:S??x}):ke??x),z=r??``,B=!O&&(A||ke!=null),ne=O&&C&&N==null,re=M||P&&te==null&&!L||y===R,";
