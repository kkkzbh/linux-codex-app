...
const browserApprovalPolicyMethods = [
  ...
  "async linuxReadBrowserUseState(){try{return await Vm()}catch(e){dk().warning(`Linux browser-use state unavailable`,{safe:{},sensitive:{error:e}});return{approvalMode:`alwaysAsk`,historyApprovalMode:`alwaysAsk`,downloadApprovalMode:`alwaysAsk`,uploadApprovalMode:`alwaysAsk`,allowedOrigins:[],deniedOrigins:[],allowedDownloadOrigins:[],deniedDownloadOrigins:[],allowedUploadOrigins:[],deniedUploadOrigins:[]}}}",
  ...
  "async linuxRememberBrowserApproval(e){try{e.kind===`origin`?await Gm(`allowed`,e.origin):e.kind===`fileTransfer`&&await Km(e.transferKind,`allowed`,e.origin)}catch(t){dk().warning(`Linux browser-use approval persist failed`,{safe:{kind:e.kind,origin:e.origin??null,transferKind:e.transferKind??null},sensitive:{error:t}})}}",
];
