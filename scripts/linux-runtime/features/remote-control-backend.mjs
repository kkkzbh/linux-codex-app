...
const browserUsePatchedDesktopFeaturesRegex = new RegExp(
  String.raw`function (?<fn>${IDENTIFIER})\((?<settings>${IDENTIFIER}),\{buildFlavor:(?<buildFlavor>${IDENTIFIER})=(?<buildModule>${IDENTIFIER})\.O\.resolve\(\),env:(?<env>${IDENTIFIER})=(?<processModule>${IDENTIFIER})\.default\.env,platform:(?<platform>${IDENTIFIER})=\k<processModule>\.default\.platform\}=\{\}\)\{let (?<features>${IDENTIFIER})=\k<platform>===\`linux\`&&\k<env>\.CODEX_ELECTRON_ENABLE_LINUX_BROWSER_USE===\`1\`\?\{\.\.\.\k<settings>,browserPane:!0,inAppBrowserUse:!0,inAppBrowserUseAllowed:!0,externalBrowserUse:!0,externalBrowserUseAllowed:!0\}:\k<platform>===\`win32\`&&\k<env>\.CODEX_ELECTRON_ENABLE_WINDOWS_COMPUTER_USE===\`1\`\?\{\.\.\.\k<settings>,computerUse:!0,computerUseNodeRepl:!0\}:\k<settings>,(?<devOverrides>${IDENTIFIER})=\k<buildFlavor>===\k<buildModule>\.O\.Dev\?(?<readEnvOverrides>${IDENTIFIER})\(\k<env>\):null;return \k<devOverrides>==null\?\k<features>:\{\.\.\.\k<features>,\.\.\.\k<devOverrides>\}\}`,
);
...
function replaceRemoteControlDesktopFeatures(...args) {
  ...
  return `function ${fn}(${settings},{buildFlavor:${buildFlavor}=${buildModule}.O.resolve(),env:${env}=${processModule}.default.env,platform:${platform}=${processModule}.default.platform}={}){let ${features}=${platform}===\`linux\`?{...${settings},control:!0,...${env}.CODEX_ELECTRON_ENABLE_LINUX_BROWSER_USE===\`1\`?{browserPane:!0,inAppBrowserUse:!0,inAppBrowserUseAllowed:!0,externalBrowserUse:!0,externalBrowserUseAllowed:!0}:{}}:${platform}===\`win32\`&&${env}.CODEX_ELECTRON_ENABLE_WINDOWS_COMPUTER_USE===\`1\`?{...${settings},computerUse:!0,computerUseNodeRepl:!0}:${settings},${devOverrides}=${buildFlavor}===${buildModule}.O.Dev?${readEnvOverrides}(${env}):null;return ${devOverrides}==null?${features}:{...${features},...${devOverrides}}}`;
}
