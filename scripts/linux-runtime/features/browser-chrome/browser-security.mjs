...
const browserSecurityMethodsAnchor =
  "function Je({setBrowserUseNativePipeEnabled:e}){return{setDesktopFeatureAvailability:t=>{t.inAppBrowserUse!=null&&e(t.inAppBrowserUse)},dispose:()=>{e(!1)}}}";
...
const browserSecurityMethods =
  ...
  "async function codexLinuxReadBrowserUseState(e){try{return await ny(e)}catch(e){t.Vr().warning(`Linux browser-use state unavailable`,{safe:{},sensitive:{error:e}});return{approvalMode:`alwaysAsk`,historyApprovalMode:`alwaysAsk`,downloadApprovalMode:`alwaysAsk`,uploadApprovalMode:`alwaysAsk`,allowedOrigins:[],deniedOrigins:[],allowedDownloadOrigins:[],deniedDownloadOrigins:[],allowedUploadOrigins:[],deniedUploadOrigins:[]}}}" +
  ...
  "async function codexLinuxRememberBrowserApproval(e,n){try{e.kind===`origin`?await sy(`allowed`,e.origin,n):e.kind===`fileTransfer`&&await cy(e.transferKind,`allowed`,e.origin,n)}catch(n){t.Vr().warning(`Linux browser-use approval persist failed`,{safe:{kind:e.kind,origin:e.origin??null,transferKind:e.transferKind??null},sensitive:{error:n}})}}" +
  ...
  "async function codexLinuxAuthenticatedFetch(e,t){let n=new URL(String(e?.url??``)),i=String(e?.method??`GET`).toUpperCase();if(n.origin!==`https://chatgpt.com`||n.pathname!==`/backend-api/aura/site_status`||i!==`GET`)throw Error(`Linux authenticated fetch URL is not allowlisted`);let a=await Bh({action:`perform Linux authenticated browser fetch`,appServerClient:t.appServerClient,desktopOriginator:Hl,headers:{},refreshToken:!1}),o=await r.net.fetch(n.toString(),{method:`GET`,headers:a});if(o.status===401){a=await Bh({action:`perform Linux authenticated browser fetch`,appServerClient:t.appServerClient,desktopOriginator:Hl,headers:{},refreshToken:!0}),o=await r.net.fetch(n.toString(),{method:`GET`,headers:a})}let s={};o.headers.forEach((e,t)=>{s[t]=e});let c=Buffer.from(await o.arrayBuffer()).toString(`base64`);return{status:o.status,statusText:o.statusText,headers:s,bodyBase64:c}}" +
...
export const browserSecurityFeature = {
  ...
  apply(bundleSources) {
    ...
    main = replaceOrThrow(
      main,
      "let Ce=Je({setBrowserUseNativePipeEnabled:e=>{de.getBrowserSessionRegistry().setBrowserUseNativePipeEnabled(e)}});O.add(()=>{Ce.dispose(),be.dispose()});",
      "let Ce=Je({setBrowserUseNativePipeEnabled:e=>{de.getBrowserSessionRegistry().setBrowserUseNativePipeEnabled(e)}}),codexLinuxBrowserSecurity=codexLinuxStartBrowserSecurityServers({appServerClient:de.getAppServerClient(),codexHome:A.codexHome});O.add(()=>{Ce.dispose(),be.dispose(),codexLinuxBrowserSecurity.dispose()});",
